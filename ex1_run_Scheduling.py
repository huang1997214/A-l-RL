...
from agent_RL import DQN
from agent_others import baselines
from RL.model import myDQN, RL_model
import torch
import numpy as np
lamda = [25]
changeTimes = len(lamda)
EPISODE = 10
policyNum = 7
policyName = ['my_dqn', 'round-robin', 'earliest', 'DQN', 'best-fit', 'sensible', 'ad_dqn']
start_learn = 500  # DQN parameter
learn_interval = 1  # DQN parameter
my_learn_interval = 1
performance_lamda = np.zeros((changeTimes, policyNum))
performance_success = np.zeros((changeTimes, policyNum))
performance_util = np.zeros((changeTimes, policyNum))
performance_finishT = np.zeros((changeTimes, policyNum))
my_start_learn = 500
...
t_start = time.time()
for changeT in range(changeTimes):
    print('----------------------------lamda: ', lamda[changeT], '----------------------------')
    env = SchedulingEnv(lamda[changeT])
    brainRL = DQN(env.actionNum, env.s_features)
    mod = RL_model(11, 10)
    ...
    for episode in range(EPISODE):
        ...
        job_c = 1  # job counter
        performance_c = 0
        env.reset(lamda[changeT])  # attention: whether generate new workload, if yes, don't forget to modify reset() function
        brainOthers.sensible_reset()
        performance_respTs = []

        while True:
            global_step += 1
            finish, job_attrs = env.workload(job_c)  # job_attrs = [id, arrival_time, length, type, ddl]
            # print('job_attrs:', job_attrs)
            # DQN policy
            DQN_state = env.getState(job_attrs, 4)  # job type, VM wait time
            ...
            # suitable policy
            # action_suit = brainOthers.suit_choose_action(job_attrs)  # random
            ...
            state_sensible = env.getStateP(job_attrs[0])  # return: job duration    job_attrs[0]: job id
            ...
            if job_c % 500 == 0:
                ...
                if (changeT == changeTimes-1) and (episode == EPISODE-1):
        ...
        print('[sensible policy] reward:', total_Rewards[6], ' avg_responseT:', avg_allRespTs[6], '',
              'success_rate:', total_success[6], ' utilizationRate:', avg_util[6], ' finishT:', total_Ts[6])

        if episode != 0:
            performance_lamda[changeT, :] += env.get_total_responseTs(policyNum, 0)
            performance_success[changeT, :] += env.get_totalSuccess(policyNum, 0)
            performance_util[changeT, :] += env.get_avgUtilitizationRate(policyNum, 0)
            performance_finishT[changeT, :] += env.get_totalTimes(policyNum, 0)
    print('')
...
'''
# pic 2: the successRate in one episode (line pic)
...
x_sticks = np.linspace(0, draw_success.shape[0] - 1, draw_success.shape[0])
# get performance according to jobNum (if job_c % N == 0: , N >=300)
# x_sticks_names = np.linspace(1, draw_success.shape[0], draw_success.shape[0])
# get performance according to time
