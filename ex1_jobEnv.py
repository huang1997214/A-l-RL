...
class SchedulingEnv:
    def __init__(self, lamda):
        ...
        self.mu = 10  # each VM service rate
        # -------------------------- state & action space -------------------------------
    ...
    def gen_workload(self, lamda):
        ...
        '''
        # (according to mu, exponential distribution)
        lengths = stats.expon.rvs(scale=1 / self.mu, size=self.jobNum)
        self.lengths = lengths
        print("length mean: ", round(np.mean(lengths), 3), '  length SD:', round(np.std(lengths, ddof=1), 3))
        '''
    ...
    def feedback(self, job_attrs, action, policyID):
        ...
        #my_reward = np.maximum(1 - (durationT - length) * 0.05, 0)
