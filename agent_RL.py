# state: 1- job attributes(length, cpu)
#        2- each VM (wait_T or wait_T rank, waste%)
import numpy as np
import random
...
# !!!!! don't forget:  w_initializer also specifies seed = 5
...
class DQN:
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            ...
            if done:
                q_real_batch.append(minibatch[i][2])
            else:
                q_real_batch.append(minibatch[i][2] + self.gamma * np.max(q_next[i]))
            """
